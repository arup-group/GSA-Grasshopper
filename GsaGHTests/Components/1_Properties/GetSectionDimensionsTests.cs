...
namespace GsaGHTests.Components.Properties
{
  ...
  public class GetSectionDimensionsTests
  {
    ...
    [Theory]
    [InlineData("STD A(cm) 25 30 4.5 6.7", 25, 30, 30, 30, 6.7, 6.7, 4.5, 0, 0, LengthUnit.Centimeter)]
    [InlineData("STD CH 100 200 10 15", 100, 200, 200, 200, 15, 15, 10, 0, 0, LengthUnit.Millimeter)]
    ...
    [InlineData("STD GZ 300 150 200 15 10 5", 300, 350, 150, 200, 15, 10, 5, 0, 0, LengthUnit.Millimeter)]
    [InlineData("STD GI 900 200 400 15 40 30", 900, 400, 200, 400, 40, 30, 15, 0, 0, LengthUnit.Millimeter)]
    ...
    [InlineData("STD RE 400 250 350 200 2", 400, 350, 250, 200, 0, 0, 0, 0, 0, LengthUnit.Millimeter)]
    ...
    [InlineData("STD SHT 500 600 150 250 15 10", 500, 600, 150, 250, 15, 15, 10, 0, 0, LengthUnit.Millimeter)]
    ...
    public void CreateComponent(string profile, double expectedDepth, double expectedWidth, double expectedWidthTop, double expectedWidthBottom, double expectedFlngThkTop, double expectedFlngThkBtm, double expectedWebThk, double expectedRootRadius, double expectedSpacing, LengthUnit unit)
    {
      var comp = ComponentMother();
      ComponentTestHelper.SetInput(comp, profile, 0);

      int i = 0;
      GH_UnitNumber depth = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber width = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber widthTop = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber widthBottom = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber flngThkTop = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber flngThkBot = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber webThk = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber radius = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
      GH_UnitNumber spacing = (GH_UnitNumber)ComponentTestHelper.GetOutput(comp, i++);
