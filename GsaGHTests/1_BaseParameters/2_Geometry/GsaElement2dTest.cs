...
namespace GsaGHTests.Parameters {
  ...
  public class GsaElement2dTests {
    ...
    public void TestCreateGsaElem2dFromMesh() {
      ...
      var elem = new GsaElement2d(mesh, false);
      int elid = 14;
      int secid = 3;
      ...
      int chsecid = 3;
      for (int i = 0; i < elem.ApiElements.Count; i++) {
        var feElements = elem.ApiElements[i] as Element;
        if (mesh.Faces[i].IsTriangle) {
          Assert.True(feElements.Type == ElementType.TRI3);
        }

        if (mesh.Faces[i].IsQuad) {
          Assert.True(feElements.Type == ElementType.QUAD4);
        }
        ...
        if (feElements.Type == ElementType.QUAD4) {
        ...
        Assert.Equal(22, feElements.Group);
        Assert.True(feElements.IsDummy);
        Assert.Equal("Shahin", feElements.Name);
    ...
    public void TestDuplicateElem2d() {
      ...
      for (int i = 0; i < dup.ApiElements.Count; i++) {
        var feElement = dup.ApiElements[i] as Element;
        if (mesh.Faces[i].IsTriangle) {
          Assert.True(feElement.Type == ElementType.TRI3);
        }

        if (mesh.Faces[i].IsQuad) {
          Assert.True(feElement.Type == ElementType.QUAD4);
        }
        ...
        if (feElement.Type != ElementType.QUAD4) {
          continue;
        }
      ...
      for (int i = 0; i < dup.ApiElements.Count; i++) {
        ...
        Assert.Equal(2, feElement.Group);
        Assert.False(feElement.IsDummy);
        Assert.Equal("Esmaeil", feElement.Name);
      ...
      checkSectId = 30;
      for (int i = 0; i < origi.ApiElements.Count; i++) {
        var feElement = origi.ApiElements[i] as Element;
        // check other members are valid
        Assert.Equal(checkId++, origi.Ids[i]);
        Assert.Equal(checkSectId++, origi.Prop2ds[i].Id);
        Assert.Equal(4, feElement.Group);
        Assert.True(feElement.IsDummy);
        Assert.Equal("Mani", feElement.Name);
    ...
    public void GetCenterPointsReturnsValidPointsForQuad4Type() {
      ...
      var element = ele.ApiElements[0] as Element;
      element.Type = ElementType.QUAD8;
      element.Type = ElementType.QUAD8;
    ...
    private GsaElement2d CreateSampleElement2dWithQuad4Type() {
      ...
      elem.ApiElements.SetMembers(nms);
      elem.ApiElements.SetMembers(off);
      var element = elem.ApiElements[0] as Element;
      element.Type = ElementType.QUAD4;
      element.Topology = new ReadOnlyCollection<int>(new List<int>(4) { 1, 2, 3, 4 });
      element.Type = ElementType.QUAD4;
      element.Topology = new ReadOnlyCollection<int>(new List<int>(4) { 4, 3, 2, 1 });
