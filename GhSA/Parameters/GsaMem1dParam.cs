...
namespace GhSA.Parameters
{
    ...
    public class GsaMember1dGoo : GH_GeometricGoo<GsaMember1d>, IGH_PreviewData
    {
        ...
        public void DrawViewportWires(GH_PreviewWireArgs args)
        {
            ...
            if (!Value.Member.IsDummy)
            {
                DrawReleases(args, Value);
            }
        }

        private void DrawReleases(GH_PreviewWireArgs args, GsaMember1d member)
        {
            #region translation start
            if (member.ReleaseStart.X == true)
            {
                Point3d pt;
                double scale = 1;
                if (member.PolyCurve.GetLength() < 1)
                {
                    pt = Value.PolyCurve.PointAtNormalizedLength(0.05);
                    scale = member.PolyCurve.GetLength();
                }
                else
                    pt = Value.PolyCurve.PointAtLength(0.05);

                Plane pln = new Plane();
                member.PolyCurve.PerpendicularFrameAt(0.02, out pln);
                pln.Rotate(member.Member.OrientationAngle, pln.Normal);
            ...
            if (member.ReleaseEnd.X == true)
            {
                Point3d pt;
                double scale = 1;
                if (member.PolyCurve.GetLength() < 1)
                {
                    pt = Value.PolyCurve.PointAtNormalizedLength(0.95);
                    scale = member.PolyCurve.GetLength();
                }
                else
                {
                    double len = member.PolyCurve.GetLength();
                    pt = Value.PolyCurve.PointAtLength(len - 0.05);
                }

                Plane pln = new Plane();
                member.PolyCurve.PerpendicularFrameAt(0.98, out pln);
                pln.Rotate(member.Member.OrientationAngle, pln.Normal);
                Vector3d vec1 = new Vector3d(pln.XAxis);
                vec1.Unitize();
                vec1 = new Vector3d(vec1.X * 0.025 * scale, vec1.Y * 0.025 * scale, vec1.Z * 0.025 * scale);
            ...
            if (member.ReleaseStart.XX == true)
            {
                Point3d pt;
                double scale = 1;
                if (member.PolyCurve.GetLength() < 1)
            ...
            if (member.ReleaseStart.YY == true)
            {
                Point3d pt;
                double scale = 1;
                if (member.PolyCurve.GetLength() < 1)
            ...
            if (member.ReleaseStart.ZZ == true)
            {
                Point3d pt;
                double scale = 1;
                if (member.PolyCurve.GetLength() < 1)
        ...
        private void DrawRelease(GH_PreviewWireArgs args, PolyCurve polyCurve, double orientationAngle)
        {
