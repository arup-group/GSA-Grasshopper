    ...
    public class GetGeometry_OBSOLETE : GH_Component, IGH_PreviewObject, IGH_VariableParameterComponent
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (DA.GetData(0, ref gh_typ))
            {
                ...
                ConcurrentDictionary<int, Node> out_nDict = (nodeList == "all") ? nDict : new ConcurrentDictionary<int, Node>(model.Nodes(nodeList));

                // create nodes
                List<GsaNodeGoo> nodes = Util.Gsa.FromGSA.GetNodes(out_nDict, axDict);
                // create elements
