...
namespace GsaGH.Helpers.Export {
  internal class ElementListFromReference {
    ...
    internal static string GetReferenceElementIdsDefinition(
      ...
      return GetReferenceDefinition(load._refObjectGuid, load._referenceType, model.Properties,
        model.Elements, model.Members, model.MemberElementRelationship);
    }
    ...
    internal static string GetReferenceElementIdsDefinition(GsaFaceLoad load, ModelAssembly model) {
      return GetReferenceDefinition(load._refObjectGuid, load._referenceType, model.Properties,
        model.Elements, model.Members, model.MemberElementRelationship);
    }

    internal static string GetReferenceElementIdsDefinition(GsaBeamLoad load, ModelAssembly model) {
      return GetReferenceDefinition(load._refObjectGuid, load._referenceType, model.Properties,
        model.Elements, model.Members, model.MemberElementRelationship);
    }
    ...
    private static string GetElementsReferenceDefinition<T>(
      Guid guid, GsaGuidIntListDictionary<T> dictionary) {
    ...
    private static string GetMemberChildElementsReferenceDefinition<T>(
      ...
      return !dictionary.GuidDictionary.TryGetValue(guid, out int id) ? string.Empty :
        GetMemberChildElementReferenceIdsDefinition(id, memberElementRelationship);
    }

    internal static string GetReferenceDefinition(
      Guid guid, ReferenceType referenceType, Properties apiProperties,
      GsaGuidIntListDictionary<Element> apiElements, GsaGuidDictionary<Member> apiMembers,
      ConcurrentDictionary<int, ConcurrentBag<int>> memberElementRelationship) {
      switch (referenceType) {
        case ReferenceType.Property: return apiProperties.GetReferenceDefinition(guid);

        case ReferenceType.Element: return GetElementsReferenceDefinition(guid, apiElements);

        case ReferenceType.MemberChildElements:
          return GetMemberChildElementsReferenceDefinition(guid, apiMembers,
            memberElementRelationship);

        case ReferenceType.Member:
          return apiMembers.GuidDictionary.TryGetValue(guid, out int id) ? id.ToString() :
            string.Empty;
