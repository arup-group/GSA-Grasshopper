...
namespace GsaGH.Parameters {
  ...
  public class GsaResultParameter : GH_OasysPersistentParam<GsaResultGoo> {
    ...
    public override string InstanceDescription
      => m_data.DataCount == 0 ? $"Empty {GsaResultGoo.Name} parameter" : base.InstanceDescription;
    ...
    protected override GsaResultGoo PreferredCast(object data) {
      if (data.GetType() == typeof(GsaModelGoo)) {
        this.AddRuntimeError(
          $"Use 'SelectResults' component to pick results case.{Environment.NewLine}Data conversion failed from {data.GetTypeName()} to Result");
      } else {
