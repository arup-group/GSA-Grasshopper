...
namespace GsaGH.Components {
  ...
  public class NodeContourResults_OBSOLETE : GH_OasysDropDownComponent {
    ...
    public NodeContourResults_OBSOLETE() : base("Node Contour Results", "ContourNode",
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddTextParameter("Node filter list", "No",
        $"Filter results by list.{Environment.NewLine}Node list should take the form:{Environment.NewLine} 1 11 to 72 step 2 not (XY3 31 to 45){Environment.NewLine}Refer to GSA help file for definition of lists and full vocabulary.",
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Point", "P", "Contoured Points with result values",
        GH_ParamAccess.list);
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      Parallel.ForEach(gsanodes, node => {
        ...
        switch (_mode) {
          case FoldMode.Displacement:
            ...
            switch (_disp) {
              case DisplayValue.X:
                t = xyzResults[nodeId][0].X.ToUnit(_lengthUnit);
                translation.X = t.Value * _defScale;
                break;

              case DisplayValue.Y:
                t = xyzResults[nodeId][0].Y.ToUnit(_lengthUnit);
                translation.Y = t.Value * _defScale;
                break;

              case DisplayValue.Z:
                t = xyzResults[nodeId][0].Z.ToUnit(_lengthUnit);
      ...
      for (int i = 0; i < ghGradient.GripCount; i++) {
        ...
        switch (_mode) {
          ...
          case FoldMode.Reaction: {
            ...
            _legendValues.Add(
              $"{t.ToString($"F{significantDigits}")} {Moment.GetAbbreviation(DefaultUnits.MomentUnit)}");
      ...
      da.SetDataList(0, pointResultGoos.OrderBy(x => x.Key).Select(y => y.Value).ToList());
      da.SetDataList(1, cs);
      da.SetDataList(2, ts);
    }
