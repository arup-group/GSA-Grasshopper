...
namespace GsaGH.Components {
  ...
  public class Elem1dContourResults_OBSOLETE : GH_OasysDropDownComponent {
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Line",
        "L",
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      elementIDs = result.Type == GsaResult.CaseType.AnalysisCase
        ? result.ACaseElement1DResults.Values.First()
          .Select(x => x.Key)
          .ToList()
      ...
      double dmaxY = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxY.As(xyzunit);
      double dmaxZ = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxZ.As(xyzunit);
      double dmaxXyz = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxXyz.As(xyzunit);
      double dminX = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminX.As(xyzunit);
      double dminY = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminY.As(xyzunit);
      double dminZ = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminZ.As(xyzunit);
      double dminXyz = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminXyz.As(xyzunit);
      double dmaxXx = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxXx.As(xxyyzzunit);
      double dmaxYy = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxYy.As(xxyyzzunit);
      double dmaxZz = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxZz.As(xxyyzzunit);
      double dmaxXxyyzz = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DmaxXxyyzz.As(xxyyzzunit);
      double dminXx = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminXx.As(xxyyzzunit);
      double dminYy = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminYy.As(xxyyzzunit);
      double dminZz = _mode == FoldMode.StrainEnergy
        ? 0
        : res.DminZz.As(xxyyzzunit);
      double dminXxyyzz = _mode == FoldMode.StrainEnergy
        ? 0
          ...
          for (int i = 0; i < positionsCount - 1; i++) {
            ...
            switch (_mode) {
              ...
              case FoldMode.StrainEnergy:
                if (_disp == DisplayValue.X) {
                  t1 = xyzResults[key][i]
                    .X.ToUnit(DefaultUnits.EnergyUnit);
                  t2 = xyzResults[key][i + 1]
                    .X.ToUnit(DefaultUnits.EnergyUnit);
                }
                else {
