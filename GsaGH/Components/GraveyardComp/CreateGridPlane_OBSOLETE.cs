...
namespace GsaGH.Components {
  ...
  public class CreateGridPlane_OBSOLETE : GH_OasysDropDownComponent {
    ...
    protected override System.Drawing.Bitmap Icon => Properties.Resources.GridPlane;
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Grid Elevation [" + Length.GetAbbreviation(_lengthUnit) + "]", "Ev", "Grid Elevation (Optional). Note that this value will be added to Plane origin location in the plane's normal axis direction.", GH_ParamAccess.item);
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      if (_mode == FoldMode.General)
        ...
        if (Params.Input[4].SourceCount > 0)
          gsaGridPlaneSurface.GridPlane.ToleranceAbove = Input.UnitNumber(this, da, 4, _lengthUnit, true).As(LengthUnit.Meter);

        if (Params.Input[5].SourceCount > 0)
          gsaGridPlaneSurface.GridPlane.ToleranceBelow = Input.UnitNumber(this, da, 5, _lengthUnit, true).As(LengthUnit.Meter);
      }
