...
namespace GsaGH.Components {
  ...
  public class GhAnalyse : GH_OasysDropDownComponent {
    ...
    protected override System.Drawing.Bitmap Icon => Properties.Resources.Analyse;
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      model.Model = Helpers.Export.AssembleModel.Assemble(model, nodes, elem1ds, elem2ds, elem3ds, mem1ds, mem2ds, mem3ds, sections, prop2Ds, prop3Ds, loads, gridPlaneSurfaces, analysisTasks, combinationCases, _lengthUnit, _tolerance, _reMesh, this);
      ...
      if (_analysis) {
        ...
        if (tryAnalyse) {
          if (!SolverRequiredDll.IsCorrectVersionLoaded()) {
            tryAnalyse = false;
            string message = "A dll required to run analysis has been previously loaded by another application. Please remove this file and try again:" + Environment.NewLine
             + Environment.NewLine + SolverRequiredDll.loadedFromPath
    ...
    private List<string> _checkboxTexts = new List<string>() { "Analyse task(s)", "ElemsFromMems" };
    private List<bool> _initialCheckState = new List<bool>() { true, true };
    ...
    public override bool Read(GH_IO.Serialization.GH_IReader reader) {
      ...
      _initialCheckState = new List<bool>() { _analysis, _reMesh };
