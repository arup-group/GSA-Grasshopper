  ...
  public class Elem1dContourResults : GH_OasysDropDownComponent {
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      if (da.GetDataList(3, ghColours)) {
        foreach (GH_Colour ghColour in ghColours)
        {
          GH_Convert.ToColor(ghColour, out Color color, GH_Conversion.Both);
          colors.Add(color);
        }
      }
      ...
      for (int i = 0; i < gHGradient.GripCount; i++) {
        double t = dmin + (dmax - dmin) / (gHGradient.GripCount - 1) * i;
        if (t > 1) {
        ...
        Color gradientcolour = gHGradient.ColourAt(2 * i / ((double)gHGradient.GripCount - 1) - 1);
