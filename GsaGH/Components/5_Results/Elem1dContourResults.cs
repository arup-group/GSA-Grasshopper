  ...
  public class Elem1dContourResults : GH_OasysDropDownComponent {
    ...
    public override void VariableParameterMaintenance() {
      ...
      switch (_mode) {
        ...
        case FoldMode.Force when (int)_disp < 4:
          Params.Output[2]
            .Name = "Legend Values [" + Force.GetAbbreviation(_forceUnit) + "]";
          break;

        case FoldMode.Force:
          Params.Output[2]
            .Name = "Legend Values [" + Moment.GetAbbreviation(_momentUnit) + "]";
          break;

        case FoldMode.StrainEnergy:
          Params.Output[2]
            .Name = "Legend Values [" + Energy.GetAbbreviation(_energyResultUnit) + "]";
          break;
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddIntegerParameter("Intermediate Points",
        "nP",
        "Number of intermediate equidistant points (default 10)",
        GH_ParamAccess.item,
        10);
      ...
      pManager.AddIntervalParameter("Min/Max Domain",
        ...
        GH_ParamAccess.item);
      pManager.AddNumberParameter("Scale",
        "x:X",
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Result Line",
        "L",
        "Contoured Line segments with result values",
        GH_ParamAccess.tree);
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      switch (_mode) {
        case FoldMode.Displacement:
          res = result.Element1DDisplacementValues(elementlist, positionsCount, _lengthResultUnit)
            [0];
          break;

        case FoldMode.Force:
          res = result.Element1DForceValues(elementlist,
            positionsCount,
            _forceUnit,
            _momentUnit)[0];
          break;
        ...
        case FoldMode.StrainEnergy:
          res = _disp == DisplayValue.X
            ? result.Element1DStrainEnergyDensityValues(elementlist,
              positionsCount,
              _energyResultUnit)[0]
      ...
      double dmaxY = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxY.As(xyzunit);
      double dmaxZ = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxZ.As(xyzunit);
      double dmaxXyz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxXyz.As(xyzunit);
      double dminX = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminX.As(xyzunit);
      double dminY = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminY.As(xyzunit);
      double dminZ = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminZ.As(xyzunit);
      double dminXyz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminXyz.As(xyzunit);
      double dmaxXx = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxXx.As(xxyyzzunit);
      double dmaxYy = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxYy.As(xxyyzzunit);
      double dmaxZz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxZz.As(xxyyzzunit);
      double dmaxXxyyzz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DmaxXxyyzz.As(xxyyzzunit);
      double dminXx = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminXx.As(xxyyzzunit);
      double dminYy = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminYy.As(xxyyzzunit);
      double dminZz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        : res.DminZz.As(xxyyzzunit);
      double dminXxyyzz = _mode == FoldMode.StrainEnergy || _mode == FoldMode.Footfall
        ? 0
        ...
        element => {
          ...
          for (int i = 0; i < positionsCount - 1; i++) {
            ...
            switch (_mode) {
              case FoldMode.Displacement:
                switch (_disp) {
                  case (DisplayValue.X):
                    t1 = xyzResults[key][i]
                      .X.ToUnit(_lengthResultUnit);
                    t2 = xyzResults[key][i + 1]
                      .X.ToUnit(_lengthResultUnit);
                    startTranslation.X = xyzResults[key][i]
                        .X.As(lengthUnit)
                      * _defScale;
                    endTranslation.X = xyzResults[key][i + 1]
                        .X.As(lengthUnit)
                      * _defScale;
                    break;
                  ...
                  case (DisplayValue.Y):
                    t1 = xyzResults[key][i]
                      .Y.ToUnit(_lengthResultUnit);
                    t2 = xyzResults[key][i + 1]
                      .Y.ToUnit(_lengthResultUnit);
                    startTranslation.Y = xyzResults[key][i]
                        .Y.As(lengthUnit)
                      * _defScale;
                    endTranslation.Y = xyzResults[key][i + 1]
                        .Y.As(lengthUnit)
                      * _defScale;
                    break;
                  ...
                  case (DisplayValue.Z):
                    t1 = xyzResults[key][i]
                      .Z.ToUnit(_lengthResultUnit);
                    t2 = xyzResults[key][i + 1]
                      .Z.ToUnit(_lengthResultUnit);
                    startTranslation.Z = xyzResults[key][i]
                        .Z.As(lengthUnit)
                      * _defScale;
                    endTranslation.Z = xyzResults[key][i + 1]
                        .Z.As(lengthUnit)
                      * _defScale;
                    break;
              ...
              case FoldMode.Footfall:
                t1 = xyzResults[key][i]
                  .X.ToUnit(RatioUnit.DecimalFraction);
                t2 = xyzResults[key][i]
                  .X.ToUnit(RatioUnit.DecimalFraction);
                break;
            }
            ...
            Color valcol1 = double.IsNaN(tnorm1)
              ? Color.Black
              : ghGradient.ColourAt(tnorm1);
            Color valcol2 = double.IsNaN(tnorm2)
              ? Color.Black
              : ghGradient.ColourAt(tnorm2);
