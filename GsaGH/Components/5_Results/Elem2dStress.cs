  ...
  public class Elem2dStress : GH_OasysDropDownComponent {
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddNumberParameter("Stress Layer", "σL",
        $"Layer within the cross-section to get results.{Environment.NewLine}Input a number between -1 and 1, representing the normalised thickness,{Environment.NewLine}default value is zero => middle of the element.",
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      string note
        = $"{Environment.NewLine}DataTree organised as {{ CaseID ; Permutation ; ElementID }} {Environment.NewLine}fx. {{1;2;3}} is Case 1, Permutation 2, Element 3, where each {Environment.NewLine}branch contains a list of results in the following order:{Environment.NewLine}Vertex(1), Vertex(2), ..., Vertex(i), Centre{Environment.NewLine}+ve in-plane stresses: tensile(ie. + ve direct strain).{Environment.NewLine}+ve bending stress gives rise to tension on the top surface.{Environment.NewLine}+ve shear stresses: +ve shear strain.";
