...
namespace GsaGH.Components {
  ...
  public class GetMaterialProperties : GH_OasysComponent, IGH_VariableParameterComponent {
    ...
    protected override System.Drawing.Bitmap Icon => Properties.Resources.MaterialProperties;
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      pManager.AddGenericParameter("Elastic Modulus [" + Pressure.GetAbbreviation(_stressUnit) + "]", "E", "Elastic Modulus of the elastic isotropic material", GH_ParamAccess.item);
      pManager.AddNumberParameter("Poisson's Ratio", "ν", "Poisson's Ratio of the elastic isotropic material", GH_ParamAccess.item);
      pManager.AddGenericParameter("Density [" + Density.GetAbbreviation(_densityUnit) + "]", "ρ", "Density of the elastic isotropic material", GH_ParamAccess.item);
      CoefficientOfThermalExpansionUnit temp = UnitsHelper.GetCoefficientOfThermalExpansionUnit(_temperatureUnit);
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      ...
      var eModulus = new Pressure(gsaMaterial.AnalysisMaterial.ElasticModulus, PressureUnit.Pascal); //create unit from SI as API is in SI units
      ...
      var density = new Density(gsaMaterial.AnalysisMaterial.Density, DensityUnit.KilogramPerCubicMeter);//create unit from SI as API is in SI units
      density = new Density(density.As(_densityUnit), _densityUnit);
      da.SetData(2, new GH_UnitNumber(density));

      var deltaT = new CoefficientOfThermalExpansion(gsaMaterial.AnalysisMaterial.CoefficientOfThermalExpansion, CoefficientOfThermalExpansionUnit.InverseDegreeCelsius);//create unit from SI as API is in SI units
      CoefficientOfThermalExpansionUnit temp = UnitsHelper.GetCoefficientOfThermalExpansionUnit(_temperatureUnit);
    ...
    public override void AppendAdditionalMenuItems(ToolStripDropDown menu) {
      ...
      var temperatureUnitsMenu = new ToolStripMenuItem("Temperature") { Enabled = true };
      foreach (string unit in UnitsHelper.GetFilteredAbbreviations(EngineeringUnits.Temperature)) {
        var toolStripMenuItem = new ToolStripMenuItem(unit, null, (s, e) => { UpdateTemperature(unit); }) {
    ...
    private void UpdateMessage() {
      CoefficientOfThermalExpansionUnit temp = UnitsHelper.GetCoefficientOfThermalExpansionUnit(_temperatureUnit);
      Message =
        Pressure.GetAbbreviation(_stressUnit) + ", " +
    ...
    public override bool Read(GH_IO.Serialization.GH_IReader reader) {
      try {
        _stressUnit = (PressureUnit)UnitsHelper.Parse(typeof(PressureUnit), reader.GetString("StressUnit"));
        _densityUnit = (DensityUnit)UnitsHelper.Parse(typeof(DensityUnit), reader.GetString("DensityUnit"));
        _temperatureUnit = (TemperatureUnit)UnitsHelper.Parse(typeof(TemperatureUnit), reader.GetString("TemperatureUnit"));
      }
      catch (Exception) {
    ...
    public virtual void VariableParameterMaintenance() {
      ...
      CoefficientOfThermalExpansionUnit temp = UnitsHelper.GetCoefficientOfThermalExpansionUnit(_temperatureUnit);
      Params.Output[3].Name = "Thermal Expansion [" + CoefficientOfThermalExpansion.GetAbbreviation(temp) + "]";
    }
