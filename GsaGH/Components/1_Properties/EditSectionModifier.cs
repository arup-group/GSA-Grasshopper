...
namespace GsaGH.Components {
  ...
  public class EditSectionModifier : GH_OasysComponent, IGH_VariableParameterComponent {
    ...
    protected override System.Drawing.Bitmap Icon =>
      Properties.Resources.EditSectionModifier;
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Area Modifier", "A", "Modify the effective Area using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using an Area UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("I11 Modifier", "I11", "Modify the effective Iyy/Iuu using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("I22 Modifier", "I22", "Modify the effective Izz/Ivv using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("J Modifier", "J", "Modify the effective J using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("K11 Modifier", "K11", "Modify the effective Kyy/Kuu using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using a DecimalFraction UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("K22 Modifier", "K22", "Modify the effective Kzz/Kvv using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using a DecimalFraction UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("Volume Modifier", "V", "Modify the effective Volume/Length using either:" + Environment.NewLine + "BY using a Percentage UnitNumber (tweaking the existing value BY this percentage)" + Environment.NewLine + "TO using a VolumePerLength UnitNumber", GH_ParamAccess.item);
      ...
      pManager.AddIntegerParameter("Stress Option Type", "Str", "Set the Stress Option Type. Accepted inputs are:"
        + Environment.NewLine + "0: No calculation"
        + Environment.NewLine + "1: Use modified section properties"
        + Environment.NewLine + "2: Use unmodified section properties",
        GH_ParamAccess.item);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Area Modifier", "A", "Modified effective Area in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as an Area UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("I11 Modifier", "I11", "Modify the effective Iyy/Iuu in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("I22 Modifier", "I22", "Modify the effective Izz/Ivv in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("J Modifier", "J", "Modify the effective J in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as an AreaMomentOfInertia UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("K11 Modifier", "K11", "Modify the effective Kyy/Kuu in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as a DecimalFraction UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("K22 Modifier", "K22", "Modify the effective Kzz/Kvv in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as a DecimalFraction UnitNumber", GH_ParamAccess.item);

      pManager.AddGenericParameter("Volume Modifier", "V", "Modify the effective Volume/Length in either:" + Environment.NewLine + "BY as a Percentage UnitNumber" + Environment.NewLine + "TO as a VolumePerLength UnitNumber", GH_ParamAccess.item);
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
        ...
        if (GH_Convert.ToInt32(obj.Value, out int stress, GH_Conversion.Both)) {
          switch (stress)
          {
            ...
            case 2:
              ...
              break;
            default:
              this.AddRuntimeError("Error in " + Params.Input[11].NickName + " input: Must be either 0, 1 or 2 but is " + stress);
              return;
          }
        }
          ...
          if (stressString.ToLower().Contains("no"))
            ...
            this.AddRuntimeError("Error in " + Params.Input[11].NickName + " input: Must contain the one of the following phrases 'no', 'unmod' or 'mod' (case insensitive), but input is '" + stress + "'");
            return;
          }
        }
        else {
          this.AddRuntimeError("Error in " + Params.Input[11].NickName + " input: Must be either 0, 1 or 2 or contain the one of the following phrases 'no', 'unmod' or mod' (case insensitive), but input is " + stress + "'");
          return;
        }
      }
      ...
      da.SetData(2,
        modifier._sectionModifier.I11Modifier.Option == SectionModifierOptionType.BY
          ...
          : new GH_UnitNumber(modifier.I11Modifier.ToUnit(UnitsHelper.GetAreaMomentOfInertiaUnit(_lengthUnit))));

      da.SetData(3, modifier._sectionModifier.I22Modifier.Option == SectionModifierOptionType.BY
        ? new GH_UnitNumber(modifier.I22Modifier)
        : new GH_UnitNumber(modifier.I22Modifier.ToUnit(UnitsHelper.GetAreaMomentOfInertiaUnit(_lengthUnit))));

      da.SetData(4,
        modifier._sectionModifier.JModifier.Option == SectionModifierOptionType.BY
          ? new GH_UnitNumber(modifier.JModifier)
          : new GH_UnitNumber(modifier.JModifier.ToUnit(UnitsHelper.GetAreaMomentOfInertiaUnit(_lengthUnit))));
      ...
      da.SetData(7,
        modifier._sectionModifier.VolumeModifier.Option == SectionModifierOptionType.BY
          ...
          : new GH_UnitNumber(modifier.VolumeModifier.ToUnit(UnitsHelper.GetVolumePerLengthUnit(_lengthUnit))));
    ...
    public override bool Read(GH_IO.Serialization.GH_IReader reader) {
      ...
      _linearDensityUnit = (LinearDensityUnit)UnitsHelper.Parse(typeof(LinearDensityUnit), reader.GetString("DensityUnit"));
