...
namespace GsaGH {
  public class GsaGhInfo : GH_AssemblyInfo {
    ...
    internal static string disclaimer
      = $"{PluginName} is pre-release and under active development, including further testing to be undertaken. It is provided \"as-is\" and you bear the risk of using it. Future versions may contain breaking changes. Any files, results, or other types of output information created using {PluginName} should not be relied upon without thorough and independent checking. {PluginName} {Vers} requires {ProductName} 10.1.{MinGsaVersion} or higher installed.";
    ...
    public override string Description
      =>
        ...
        $"Official Oasys GSA Grasshopper Plugin{Environment.NewLine}{(isBeta ? disclaimer : string.Empty)}{Environment.NewLine}A licensed version of GSA 10.1.65 or later installed in C:\\Program Files\\Oasys\\GSA 10.1\\ is required to use this plugin.{Environment.NewLine}Contact oasys@arup.com to request a free trial version.{Environment.NewLine}{TermsConditions}{Environment.NewLine}{Copyright}";
    ...
    public override GH_LoadingInstruction PriorityLoad() {
      ...
      if (!File.Exists($"{InstallPath}\\GsaAPI.dll")) {
        var exception = new Exception(
          $"GsaGH requires GSA to be installed in {InstallPath}. Unable to find GsaAPI.dll. It looks like you haven't got GSA installed, or it may be installed in an unknown path. Please install or reinstall GSA in {InstallPath}, restart Rhino and load Grasshopper again.");
      ...
      try {
        ...
        if (gsaVers.FileBuildPart < GsaGhInfo.MinGsaVersion) {
          var exception = new Exception(
            $"Version {GsaGhInfo.Vers} of GSA-Grasshopper requires GSA 10.1.{GsaGhInfo.MinGsaVersion} installed. Please upgrade GSA.");
        ...
        string message
          = $"{e.Message}{Environment.NewLine}{Environment.NewLine}This may be due to clash with other referenced dll files by one of these plugins that's already been loaded: {Environment.NewLine}{loadedPlugins}{Environment.NewLine}You may try disable the above plugins to solve the issue.{Environment.NewLine}The plugin cannot be loaded.";
